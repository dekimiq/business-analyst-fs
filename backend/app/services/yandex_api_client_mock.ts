...
const MOCK_ADS: YandexAd[] = [
  ...
  },
  {
    ...
    State: 'ON',
    Status: 'ACCEPTED',
    TextAd: { Title: 'Быстрый старт', Text: 'Результат с первого дня. Узнайте больше.' },
  },
  {
    Id: 30000003,
    AdGroupId: 20000002,
...
/**
 * Генерирует псевдослучайные (но детерминированные) статы для одного дня.
 * Используем дату как seed — одна и та же дата всегда вернёт одни данные.
 */
...
export default class YandexApiClientMock implements IYandexApiClient {
  ...
  async ping(): Promise<boolean> {
    await this.delay()
    console.log('[YandexMock] ping() → true')
    return true
  }

  async getCampaigns(): Promise<YandexCampaign[]> {
    await this.delay()
    console.log('[YandexMock] getCampaigns()')
    return MOCK_CAMPAIGNS
  }

  async getAdGroups(campaignIds: number[]): Promise<YandexAdGroup[]> {
    await this.delay()
    console.log(`[YandexMock] getAdGroups(campaignIds: [${campaignIds.join(', ')}])`)
  ...
  async getDailyStats({
    ...
    while (current <= end) {
      ...
      console.log(`[YandexMock] getDailyStats(date: ${dateStr})`)
      results.push(...generateStatsForDay(dateStr))
